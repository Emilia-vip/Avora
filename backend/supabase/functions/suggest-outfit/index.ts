...
serve(async (request) => {
  ...
  try {
    ...
    const body = await request.json() as { wish?: string; weather?: string | null; gender?: string | null };
    ...
    const metaGender = typeof userData.user.user_metadata?.gender === "string"
      ? userData.user.user_metadata.gender
      : null;
    ...
    const suggestion = await suggestOutfit(wish, items, geminiApiKey, weather, gender);
